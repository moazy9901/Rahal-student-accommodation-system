<x-app-layout>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Trashed Messages</h1>
        ...
        <table class="w-full text-left">
            ...
            @forelse($messages as $message)
                <tr class="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 transition">
                    <td class="px-4 py-3 font-medium">{{ Str::limit($message->subject, 30) }}</td>
                    <td class="px-4 py-3">
                        <div class="text-sm">{{ $message->sender_name }}</div>
                        <div class="text-xs text-gray-500">{{ $message->sender_email }}</div>
                    </td>
                    <td class="px-4 py-3">
                        <div class="text-sm">{{ $message->recipient_name }}</div>
                        <div class="text-xs text-gray-500">{{ $message->recipient_email }}</div>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">{{ $message->deleted_at->format('M d, Y') }}</td>
                    <td class="px-4 py-3 flex gap-2">
                        <a href="{{ route('messages.restore', $message->id) }}"
                           class="px-3 py-1 bg-green-600 text-white rounded-lg text-sm hover:bg-green-500">
                            Restore
                        </a>

                        <form action="{{ route('messages.force-delete', $message->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button
                                type="button"
                                @click="$dispatch('confirm-delete', { url: '{{ route('messages.force-delete', $message->id) }}' })"
...
<div x-data="{ open: false, deleteUrl: '' }"
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-xl w-full max-w-md mx-4">
        <h3 class="text-lg font-semibold mb-4">Confirm Permanent Delete</h3>
        <p class="mb-6">Are you sure you want to permanently delete this message? This action cannot be undone.</p>
