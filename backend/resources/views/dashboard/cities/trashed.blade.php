<x-app-layout>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Trashed Cities</h1>
        <a href="{{ route('cities.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-500">
            Back to Cities
        </a>
    </div>
                ...
                @foreach($cities as $city)
                    <tr class="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 transition">
                        ...
                        <td class="px-4 py-3 flex gap-2">
                            ...
                            <form action="{{ route('cities.force-delete', $city->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
